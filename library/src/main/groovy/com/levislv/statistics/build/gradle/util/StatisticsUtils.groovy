...
class StatisticsUtils {
    ...
    static boolean shouldModifyClass(String className) {
        if (className.endsWith('BuildConfig')
                || className.endsWith('R')
                || className.endsWith('R2')
                || className.endsWith('StatisticsR')
                || className.contains('R$')
                || className.contains('R2$')
                || className.contains('StatisticsR$')) {
            return false
        }
        return true
    }
    ...
    static String getOpName(int opCode) {
        return getOpCodeMap().get(opCode)
    }
    ...
    static Map<Integer, String> getAccCodeMap() {
        if (accCodeMap.size() == 0) {
            ...
            map.put('ACC_OPEN', 32)
            map.put('ACC_TRANSITIVE', 32)
            map.put('ACC_VOLATILE', 64)
            map.put('ACC_BRIDGE', 64)
            map.put('ACC_STATIC_PHASE', 64)
            ...
            map.put('ACC_MANDATED', 32768)
            map.put('ACC_MODULE', 32768)
    ...
    static Map<Integer, String> getOpCodeMap() {
        if (opCodeMap.size() == 0) {
            HashMap<String, Integer> map = new HashMap<>()
            map.put('NOP', 0)
